...
public class UserController extends ApplicationController{
	...
	public static class UserForward{
		...
		public static final String MY_PROFILE_VIEW 							= "user/myProfile";			
		...
		public static final String REG_ACC_POP 					        = "popup/regAccPop";
		public static final String MY_PAN_VIEW 								= "user/myFan";				
	}
	...
	public static class UserRedirect {
		...
		public static final String RANK_PAGE 							    = "redirect:/rank/rankPage";
	...
	public String showLoginForm(
		...
		if(beforeUrl == null || beforeUrl.indexOf("login") > 0){
			beforeUrl= "/rank/rankPage";
		}
		...
		loginCommand.setBeforeUrl(beforeUrl);
	...
	public String Login(
		...
		String redirectUrl = loginCommand.getBeforeUrl();
		...
		if (getAccNo(loginCommand.getEmail())) attr.addFlashAttribute("accPop", "Y");
			...
			ModelMap model)
	{
		...
		int fanCount = fanService.getFanListAndMemberCount(me, other);
		int starCount = fanService.getFanListAndMemberCount(other, me);
		
        int sponsorAmount = workspaceService.getSponsorAmount(memberNo);
        int playListCount = playListService.getPlayListCount(memberNo);
		         
        ArrayList<FanManCommand> fanList = fanService.getFanAndMemberList(me, other, pageCount, pageOffset);
        ArrayList<FanManCommand> starList = fanService.getFanAndMemberList(other, me, pageCount, pageOffset);        
        ArrayList<PlayListCommand> playList = playListService.getPlayList(me, pageCount, pageOffset);        
        
		ProfileCommand profileCommand = userService.getProfile(memberNo);	
		profileCommand.setProfileWebPath("/music/profile/" + String.valueOf(memberNo) + 
		...
		model.addAttribute("composeCount", compositionService.getCompositionCountByComposeMemberNo(memberNo));		
		model.addAttribute("arrangeMentCount", arrangementService.getArrangementCountByComposeMemberNo(memberNo));
		model.addAttribute("vocalCount", vocalService.getVocalCountByVocalMemberNo(memberNo));
		model.addAttribute("performanceCount", performanceService.getPerformanceCountByPerformMemberNo(memberNo));		
		model.addAttribute("fanCnt", fanCount);
		model.addAttribute("starCnt", starCount);		
		model.addAttribute("playListCnt", playListCount);
		...
		return UserForward.MY_PROFILE_VIEW;
	}
...
//		DateTime aTime = new DateTime("1972-09-03");
//		DateTime bTime = new DateTime();
//	    String pattern1 = "yyyy.MM.dd HH:mm";
//		System.out.println("aTime : " + aTime.toString(pattern1));
//	    System.out.println("bTime : " + bTime.toString(pattern1));
		
		ProfileCommand profileCommand = userService.getProfile(memberNo);		
		int sponTotalCount = sponsorService.getSponListCount(memberNo);		
		int me, other;
		...
		me = memberNo;
		other = 0;						
		int fanTotalRow1 = fanService.getFanListAndMemberCount(me, other);
		/*누군가가 나를 팬으로 등록한 리스트*/		
		...
		me = 0;
		other = memberNo;		
		int fanTotalRow2 = fanService.getFanListAndMemberCount(me, other);
		/*내가 누군가를 팬으로 등록한 리스트*/	
		
		String sponLastDay = sponsorService.getSponLastDay(memberNo);
		
		if(sponLastDay == null){
			sponLastDay = "시작일시 없음.";
		}else{
			sponLastDay = sponLastDay.substring(0, 10);
		}
		
		model.addAttribute("fanTotal1", fanTotalRow1);
		model.addAttribute("fanTotal2", fanTotalRow2);
		model.addAttribute("sponTotalCount", sponTotalCount);
		model.addAttribute("sponStartDay", sponLastDay);
		model.addAttribute("sponTotalSum", sponsorService.getSponSumByMemberNo(memberNo));		
		model.addAttribute("playListTotalSum", playListService.getPlayListCount(memberNo));		
		model.addAttribute("profileCommand", profileCommand);
		return UserForward.MY_PAN_VIEW;
	}


	@ResponseBody
	@RequestMapping(value = "/remoteMyFan", method = RequestMethod.GET)
	public GenericResponse<FanManCommand> remoteMyFan(
			@RequestParam(value = "likeFor", required = false, defaultValue = "0" ) int likeFor,
			@RequestParam(value = "page", required = false, defaultValue = "1" ) int page,
			@RequestParam(value = "menu", required = false, defaultValue = "1" ) int menu,
			@RequestParam(value = "memberNo", required = false, defaultValue = "1" ) int memberNo,			
			ModelMap model)
	{
		...
		if(likeFor==0){  // 내가 남을 팬으로 등록한 리스트
			me = memberNo;
			other = 0;
		}else{			// 남이 나를 팬으로 등록한 리스트
			me = 0;
			other = memberNo;
		}

		int fanTotalRow = fanService.getFanListAndMemberCount(me, other);

		GenericResponse<FanManCommand> gr = new GenericResponse<FanManCommand>();		
		Pagination pagination = new Pagination(fanTotalRow, page, fanService.PAGE_OFFSET);
		
		ArrayList<FanManCommand> fanAndMemberList = fanService.getFanAndMemberList(me,other,
				pagination.getStartRow(),fanService.PAGE_OFFSET);

		gr.setResultCode(1);
		gr.setMessage("sucess");
		gr.setListType(menu);
		gr.setCurrentRowCount(fanAndMemberList.size());
		gr.setTotalPage(fanAndMemberList.size());
		gr.setCurrentPage(page);
		gr.setTotalRow(fanTotalRow);
		gr.setObjects(fanAndMemberList);
	...
	@ResponseBody
	@RequestMapping(value = "/remoteMySpon", method = RequestMethod.GET)
	public GenericResponse<SponsorCommand> remoteMySpon(
			@RequestParam(value = "page", required = false, defaultValue = "1" ) int page,
			@RequestParam(value = "menu", required = false, defaultValue = "1" ) int menu,
			@RequestParam(value = "memberNo", required = false, defaultValue = "0" ) int memberNo,			
			ModelMap model)
	{
		...
		int sponTotalRow = sponsorService.getSponListCount(memberNo);

		GenericResponse<SponsorCommand> gr = new GenericResponse<SponsorCommand>();		
		Pagination pagination = new Pagination(sponTotalRow, page, sponsorService.PAGE_OFFSET);

		ArrayList<SponsorCommand> SponList = 
				sponsorService.getSponList(memberNo,
						pagination.getStartRow(),sponsorService.PAGE_OFFSET);

		gr.setResultCode(1);
		gr.setMessage("sucess");
		gr.setListType(menu);
		gr.setCurrentRowCount(SponList.size());
		gr.setTotalPage(SponList.size());
		gr.setCurrentPage(page);
		gr.setTotalRow(sponTotalRow);
		gr.setObjects(SponList);
	...
	@ResponseBody
	@RequestMapping(value = "/remoteDeletePlayList", method = RequestMethod.GET)
	public void remoteDeletePlayList(
			@RequestParam(value = "playListNo", required = true, defaultValue = "1" ) int playListNo,			
			ModelMap model)
	{			
	...
	@ResponseBody
	@RequestMapping(value = "/remotePlayList", method = RequestMethod.GET)
	public GenericResponse<PlayListCommand> remotePlayList(
			@RequestParam(value = "page", required = false, defaultValue = "1" ) int page,			
			@RequestParam(value = "menu", required = false, defaultValue = "1" ) int menu,
			@RequestParam(value = "memberNo", required = false, defaultValue = "0" ) int memberNo,				
			ModelMap model)
	{		
		...
		GenericResponse<PlayListCommand> gr = new GenericResponse<PlayListCommand>();
		
		int playListTotalRow = playListService.getPlayListCount(memberNo);
		Pagination pagination = new Pagination(playListTotalRow, page, playListService.PAGE_OFFSET);		
		
		ArrayList<PlayListCommand> PlayList = playListService.getPlayList(
				memberNo, 
				pagination.getStartRow(),
				playListService.PAGE_OFFSET);

		gr.setResultCode(1);
		gr.setMessage("success");
		gr.setListType(menu);
		gr.setCurrentRowCount(PlayList.size());
		gr.setTotalPage(PlayList.size());
		gr.setCurrentPage(page);
		gr.setTotalRow(playListTotalRow);
		gr.setObjects(PlayList);
	 ...
	 */
	@ResponseBody
	@RequestMapping(value ="/getProfilePopInfo", method = RequestMethod.GET)
	public ModelMap getProfilePopInfo(@RequestParam("memberNo") int memberNo,
			ModelMap model)
	{	
		model.addAttribute("user", userService.getProfilePopInfo(memberNo));
		return model;
	}
	 ...
	 * regAccPop
	 */
	@RequestMapping(value ="/getAccNo", method = RequestMethod.GET)
	public Boolean getAccNo(String email){
		User emailUser = new User();
		Boolean ret = false;
		
		emailUser = userService.getUserByEmail(email);
		
		if (emailUser  == null)	ret = false;
			else if (emailUser.getAccNo()== null || emailUser.getAccNo().equals(""))	ret = true;		
			else ret = false;
	 ...
	 * regAccPop
	 */
	@RequestMapping(value ="/regAccPop", method = RequestMethod.GET)
	public String reqAccPop(
			@RequestParam(value = "name", required = false, defaultValue = "1" ) String name,	
			ModelMap model){
		return UserForward.REG_ACC_POP;
	}	
}
