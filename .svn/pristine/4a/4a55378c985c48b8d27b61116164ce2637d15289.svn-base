...
public class CompositionServiceImpl implements CompositionService {
	...
	public int getCompositionCount(String isSearch, String searchWord) {
		return compositionDao.getCompositionCount(isSearch, searchWord);
	}
	
	public int getCompositionCountByComposeMemberNo(int composeMemberNo){
		return compositionDao.getCompositionCountByComposeMemberNo(composeMemberNo);		
	}
	...
	public ArrayList<CompositionCommand> getCompositionCommandList(int pageCount, int pageOffset, int orderBy, String searchWord, String isSearch, int loginMemberNo) {
		return compositionDao.getCompositionCommandList(pageCount, pageOffset, orderBy, searchWord, isSearch, loginMemberNo);
	}

	@Override
	public CompositionCommand getCompositionCommandByComposeNo(int composeNo, int loginMemberNo) {
		return compositionDao.getCompositionCommandByComposeNo(composeNo, loginMemberNo);
	}
	...
	public void updateCompositionUpdateCommand(ComposeUpdateCommand composeUpdateCommand) {
		...
		/* 이전 등록된 파일첨부번호를 지우지 않기 위해 다시 로드함 */
		Composition compositionDb = getCompositionByComposeNo(composition.getComposeNo());
		composition.setComposeImage(compositionDb.getComposeImage());
		composition.setComposeScore(compositionDb.getComposeScore());
		composition.setComposeSound(compositionDb.getComposeSound());
		...
		requestComposerMessage(compositionCommand, composeArrangeCommands);
	...
	private void requestComposerMessage(CompositionCommand aCommand,
			ArrayList<ComposeArrangeCommand> avCommands) {
		
		for (ComposeArrangeCommand avCommand : avCommands){
			if (!CommonUtils.isNullOrEmpty(avCommand.getMessageContent())){
				messageService.sendCommonMessage(0,aCommand.getComposeMemberNo(),avCommand.getArrangeMemberNo(),AppConstants.TransferType.SEND,
						avCommand.getMessageType(),aCommand.getComposeNo(),avCommand.getMessageContent(),AppConstants.MessageReadStatus.NOT_READ);
			}
		}
	}
	...
	public void updateCompositionFanCount(int composeNo){
		compositionDao.updateCompositionFanCount(composeNo);		
	};
