...
public class ArrangementServiceImpl implements ArrangementService {
	...
	@Override
	public void updateArrangeUpdateCommand(ArrangeUpdateCommand arrangeUpdateCommand, int codeStatus) {
		...
		for(PerformanceCommand performanceCommand : performanceCommands){
			...
			int checkArrangeNo = performance.getArrangeNo();
			if (checkArrangeNo != 0 && updateType != AppConstants.FormUpdateType.ACTION_TYPE_DELETE) {
				updateType = AppConstants.FormUpdateType.ACTION_TYPE_UPDATE;
			}
			
			if (updateType == AppConstants.FormUpdateType.ACTION_TYPE_INSERT){
				performance.initPerformance(performance);
        ...
        int arrangeNo = arrangement.getArrangeNo();
		Arrangement arrangementTemp = arrangementDao.getArrangementByArrangeNo(arrangeNo);
		arrangement.setArrangeSound(arrangementTemp.getArrangeSound());
		arrangement.setArrangeScore(arrangementTemp.getArrangeScore());
		arrangement.setArrangeZip(arrangementTemp.getArrangeZip());
		arrangement.setArrangeImage(arrangementTemp.getArrangeImage());
		arrangement.setGuideSound(arrangementTemp.getGuideSound());
		
		if (codeStatus == 1) {
			arrangement.setArrangeStatus(AppConstants.ArrangeStatus.ARRANGING);
		} else if (codeStatus == 2) {
			arrangement.setArrangeStatus(AppConstants.ArrangeStatus.ARRANGE_COMPLETED);
			arrangement.setSaleStatus(AppConstants.SalesStatus.ING_SALE);
		}

		this.updateArrangement(arrangement);
		this.performanceBatch(insertPerformanceList,BATCH_ACTION_INSERT, arrangement);
		this.performanceBatch(updatePerformanceList,BATCH_ACTION_UPDATE, arrangement);
		this.performanceBatch(deletePerformanceList,BATCH_ACTION_DELETE, arrangement);
	...
	private void performanceBatch(ArrayList<Performance> performanceList, int batchType, Arrangement arrangement){
		for (Performance performance : performanceList){
			int performNo = performance.getPerformNo();
            int performMemberNo = performance.getPerformMemberNo();

			switch(batchType){
			case BATCH_ACTION_INSERT:
				this.insertPerformance(performance);

				int maxPerformNo = performance.getPerformNo();
				PerformPlayer insertPerformPlayer = new PerformPlayer();
				insertPerformPlayer.setPerformNo(maxPerformNo);
				insertPerformPlayer.setPlayerMemberNo(performMemberNo);
				insertPerformPlayer.setPerformPlayerStatus(AppConstants.PerformParticipationStatus.REQUEST_PERFORMER);
				performPlayerService.insertPerformPlayer(insertPerformPlayer);
				break;
			case BATCH_ACTION_UPDATE:
				performance.setModMemberNo(arrangement.getArrangeMemberNo());
				
				Performance performanceUpdateCheck = arrangementDao.getPerformancesByPerformNo(performNo);
                int beforePerformMemberNo = performanceUpdateCheck.getPerformMemberNo(); 
				
				if (performance.getPerformStatus().equals(AppConstants.PerformStatus.PERFORM_UNSPECIFIED)  
						&&performance.getPerformMemberNo() != 0) {
					performance.setPerformStatus(AppConstants.PerformStatus.PERFORM_WAITING);
                    this.updatePerformance(performance);
					performance.setPerformStatus(AppConstants.PerformStatus.PERFORM_UNSPECIFIED);
				} else {
                    this.updatePerformance(performance);
				}
				
				PerformPlayerCommand updatePerformPlayer = new PerformPlayerCommand();
				updatePerformPlayer.setPerformNo(performNo);

			    if (performance.getPerformStatus().equals(AppConstants.PerformStatus.PERFORM_UNSPECIFIED) ) {
			    	updatePerformPlayer.setPlayerMemberNo(0);
			    	updatePerformPlayer.setPlayerMemberNoTemp(performMemberNo);
			    } else {
			    	updatePerformPlayer.setPlayerMemberNo(beforePerformMemberNo);
			    	updatePerformPlayer.setPlayerMemberNoTemp(performMemberNo);
			    }
				updatePerformPlayer.setPerformPlayerStatus(AppConstants.PerformParticipationStatus.REQUEST_PERFORMER);
				performPlayerService.updatePerformPlayerCommand(updatePerformPlayer);
				
				break;
			case BATCH_ACTION_DELETE:
				this.deletePerformance(performance);

				performPlayerService.deletePerformPlayer(performNo,  performMemberNo);
				break;
			}
		}
	}
	...
	public ArrayList<ArrangementCommand> getArrangementCommandList(int genreNo,
			int pageCount, int pageOffset, int orderBy, int listType) {
		return arrangementDao.getArrangementCommandList(genreNo, pageCount, pageOffset, orderBy, listType);
	}
	...
	@Override
	public int getArrangementCount(int genreNo, int listType) {
		return arrangementDao.getArrangementCount(genreNo, listType);
	}

	@Override
	public int getArrangementCountByComposeMemberNo(int composeMemberNo){
		return arrangementDao.getArrangementCountByComposeMemberNo(composeMemberNo);		
	}	
	...
	public int getArrangementCountByArrangeNo(){
		return arrangementDao.getArrangementCountByArrangeNo();
	}
