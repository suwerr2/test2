...
<form id = "arrange-update-form" name = "arrange-update-form" method ="post" >
	<input type="hidden" id="saleStatus" name="composition.saleStatus" />
	<input type="hidden" id="composeStatus" name="composition.composeStatus" />
	<input type="hidden" id="redirectUrl" name="redirectUrl" value="${command.redirectUrl}" />
	
            <div class="contents">
            	<div class="titleWrap">
					<h2 class="testBtn"><spring:message code ="title.composition"/></h2>
				</div>
                <div class="hType1">
                    <h3>곡 제목</h3>
                    <p>등록하시려는 곡의 제목을 입력해주세요.</p>
                </div>
                <div><input type="text" id="composeTitle" name="composeTitle" value="${command.compositionCommand.composeTitle}" readonly style="width: 99%"></div>
                ...
                <div class="hType1">
                    <h3>편곡자 선택<strong class="pointColor">(필수)</strong></h3>
                </div>
                <div class="arrInfoWrap">
                    <div>
						<input type="radio" name ="composition.openStatus" id="openArrange" 
							...
							<label for="openArrange">공개(누구나 편곡 가능)</label>
						...
						<label for="closedArrange">편곡자지정</label>  
                    </div>
								...
								<c:forEach items = "${command.composeArrangeCommands}" var = "composeArrangeCommand" begin = "0" 
									...
									<tr>
										...
										<td>
											...
											<input type	= "text"   name = "composeArrangeCommands[${status.index}].arrangeMemberNickName" value = "${composeArrangeCommand.arrangeMemberNickName}" id = "selectedMemberBox" class = 'selectedMemberBox' >
										</td>
										<td>
                ...
                <div class="btnAreaType01">
                	<div class="textR">
	                    <a class="btn large btnPoint" href="#none"  id ="compose-sale-begin"><span>판매개시</span></a>
	                    <a class="btn large" href="#none"  id ="compose-update-submit"><span>임시저장</span></a>
	                    <a class="btn large" href="#none"  id ="compose-cancel-submit"><span>취소</span></a>
...
<script type = "text/javascript">
	...
	$(document).ready(function(target) {
	    ...
	    //폼 등록 버튼 클릭시 호출
	    $('#compose-sale-begin').click(function(event){
...
/* 	    	if (!checkRate()){
	    	...
	    	//기타 입력갑 검증
	    	//검증이 통과할 경우 폼 제출
	    	$('#composeStatus').val("02");
	    	$('#saleStatus').val("01");
	    	$('#arrange-update-form').attr('action','/music/compose/update').submit();
	    	
	    });	 
	    //폼 등록 버튼 클릭시 호출
	    $('#compose-cancel-submit').click(function(event){
	    	event.preventDefault();
/* 	    	if (!checkRate()){
	    		alert("수익 배분은 작곡자, 편곡자 , 작사가 배분율을 더해서 \n총 100 % 가되어야 합니다");
	    		return;
	...
	function getMultiEmpInfoDeskPopAjax(init){
		 ...
		 url = "/music/common/musicianSelectList?memberUserId="+$("#searchComposer").val()+"&selectOpt="+$("#selectOpt").val();
