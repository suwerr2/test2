...
public class VocalController extends ApplicationController{
	 ...
	 */	
	@ResponseBody
	@RequestMapping(value ="/vocalRemoteInsert", method = RequestMethod.GET)
	public void vocalRemoteInsert(
			@RequestParam(value = "musicClass", required = false, defaultValue = "1") String musicClass,
			@RequestParam(value = "musicNo"	  , required = false, defaultValue = "1") int musicNo,
			@RequestParam(value = "memberNo"  , required = false, defaultValue = "0") int memberNo,			
			ModelMap model)
	{	
		Logger logger = Logger.getLogger(VocalController.class);		
		playListService.insertPlayList(musicClass, musicNo, memberNo);
	}		
	...
	public Sponsor initSponsor(
		...
		int SponAlreadyCheck = sponsorService.getSponAlreadyCheck(memberId, memberSponsorId);
		
		Sponsor sponsor = new Sponsor();
		sponsor.setMemberNo(memberId);
		sponsor.setSponsorMemberNo(memberSponsorId);
		sponsor.setSponsorAmount(sponsorAmount);
		sponsor.setCheck(SponAlreadyCheck);		
		sponsor.setSponsorNo(vocalNum);		
		
		if(SponAlreadyCheck <= 0){ //한번도 스폰하지 않은 사람 스폰 가능.			
