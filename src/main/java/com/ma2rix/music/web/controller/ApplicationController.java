...
public class ApplicationController {
	...
	/**
	 * 세션의 유저의 memberNo을 반환
	 * @return
	 */
	@ModelAttribute("memberNo")
	protected int prepareMemberNo(){
		User user  = currentUser();
		int memberNo;
		if (user == null){memberNo = 0;
		}else {memberNo = user.getMemberNo();}
		return memberNo;
	}	
	/**
	 * 세션의 유저 객체를 모델로 반환.
	...
	protected ForwardingMessage prepareForwardingMessage(ForwardingMessageType type){
		...
		message.setMainMessage(type.mainMessage());
		message.setSubMessage(type.subMessage());
		return message;
	}
